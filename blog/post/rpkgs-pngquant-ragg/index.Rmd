...
especially when it comes to passing CRAN checks. In this blog post,
...
while the `help/` directory includes PNG outputs from `README.Rmd` in the
`man/figures/` folder. To avoid this check note, my goal is to compress
...
## Final solution: optimizing PNG output with pngquant and ragg
...
My knitr chunk options are as follows
...
1. Since CRAN does not re-do `R CMD build` nor re-render vignettes and will
   reuse the built vignettes in the uploaded tarball, it is only necessary
