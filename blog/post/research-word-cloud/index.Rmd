...
- Use a word cloud generator, adjust the look, and generate an SVG file (15 min).
...
Clone this [GitHub repo](https://github.com/nanxstats/pdf-word-extraction)
to your workspace. It contains a Python script I wrote for extracting
meaningful words, as defined by a statistical model, from the PDF files:

```bash
git clone https://github.com/nanxstats/pdf-word-extraction.git
```

Enter the directory (everything below assumes you are in the directory):

```bash
cd pdf-word-extraction
```

Install the Python package dependencies of the script described in the
[setup section of the repo](https://github.com/nanxstats/pdf-word-extraction#setup).
The simple way is to use `pip` and `requirements.txt`:

```bash
pip install -r requirements.txt
```

This will install the essential toolchain in Python, including pypdf for PDF
parsing, ftfy for text cleaning, and spaCy for natural language processing (NLP).
...
dictionary `replacements`. Save and run the script again with the same command:
